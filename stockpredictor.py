...
def perf_stats(y_pred, y_test):
    ...
    y_test = lstm.denormalise_windows(y_test, actual)
    y_pred = lstm.denormalise_windows(y_pred, actual)
...
def grid_search():
    ...
    m = KerasClassifier(build_fn = lstm.build_model, layers = [1, window_size, 200, 1])
...
if __name__=='__main__':
    ...
    X_train, y_train, X_test, y_test, actual = lstm.get_data('bse_train.csv', window_size, True)

    # for TECHM stock data
    # X_train, y_train, X_test, y_test, actual = lstm.get_data('techm.csv', window_size, True)
    print('>>> Data processed...')

    #for daily predictions
    model = lstm.build_model([1, window_size, 200, 1])
    ...
    y_pred = lstm.predict_point_by_point(model, X_test)
